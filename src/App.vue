        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input
                      v-model="ticker"
                      @keyup="handleTickerInput"
                      @keypress.enter="addTicker(ticker)"
                      type="text"
                      name="wallet"
                      id="wallet"
            ...
            <div v-if="tips.length" class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span v-for="tip in tips"
                    :key="tip"
                    @click="addTicker(tip)"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            normalizeGraph() {
              ...
              return this.graph.map(
                  price => 5 + ((price - minGraphValue) * 95) / (maxGraphValue - minGraphValue)
              );
            },
            ...
            isTickerAlreadyAdd(ticker) {
                this.error = this.tickers.find(t => t.name === ticker.toUpperCase()) ? 'Такой тикер уже добавлен' : false;
            },

            isTickerValid(ticker) {
                this.error = this.allTickers.includes(ticker.toUpperCase()) ? false : 'Тикер не найден в списке';
            },
            ...
            setPrice(ticker) {
                ...
                currentTicker.price = ticker[1].USD > 1 ? ticker[1].USD.toFixed(2) : ticker[1].USD.toPrecision(2);

                if (!this.activeTicker) return;
                if (ticker[0] === this.activeTicker.name) this.graph.push(currentTicker.price);
            },

            async getPrice(tickers) {
                if (!tickers.length) return;

                const tickersString = tickers.length !== 1 ? tickers.map(t => t.name).join(',') : tickers[0].name;
        ...
        async created() {
            ...
            await fetch(url)
                .then(res => res.json())
                .then(data => this.allTickers = Object.keys(data.Data).sort((a, b) => a.length - b.length));
